...
def create_person(list)
  ...
  input = gets.chomp
  if input == '0' then create_student(list) end
  if input == '1' then create_teacher(list) end
end
...
def validate_num(input, list)
  if input.to_i > list.length - 1 || input == ""
...
def main(book_list = [], people_list = [], rentals_list = [])
  book_list = book_list
